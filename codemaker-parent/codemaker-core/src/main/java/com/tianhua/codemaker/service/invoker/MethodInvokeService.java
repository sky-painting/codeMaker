...
import com.tianhua.codemaker.enums.dynamic.InvokeLayerTypeEnum;
...
public class MethodInvokeService {
    ...
    public void dealInvokeContent(InvokeContextBean invokeBean){
        ...
        String providerClassVar = "";
        if(invokeBean.getProviderClassMethod().isStatic()){
            providerClassVar = invokeBean.getProviderClassName();
        }else {
             providerClassVar = invokeBean.getProviderClassName().substring(0,1).toLowerCase()+invokeBean.getProviderClassName().substring(1);
        }
    ...
    private String getProviderClassMethodParamStr(MethodBean invokeMethod,String providerMethodParams){
        ...
        if(CollectionUtils.isEmpty(invokeMethod.getInvokeMethodList())){
            ...
            for (int i = 0;i < providerMethodParamArr.length;i++){
                ...
                if(providerMethodParamArr[i].trim().contains(" ")){
                ...
                if(find){
                    ...
                    boolean basicType = StringHandleUtils.isBasicType(providerParamType);
                    if(basicType && providerParamType.equals("boolean")){
                        paramBuilder.append("false, ");
                    }
                    else if(basicType && providerParamType.equals("long")){
                        paramBuilder.append("0L, ");
                    }
                    else if(basicType && providerParamType.equals("int")){
     ...
     * @param annotation
     */
    public void registField(AbstractClassBean invokerClassBean,AbstractClassBean providerClassBean,String annotation){
        ...
        if(!StringUtils.isEmpty(annotation)){
            fieldBean.setAnnotation(annotation);
        }
        ...
        invokerClassBean.addField(fieldBean);
    ...
    private void  registConvertLayer(InvokeContextBean invokeBean){
        ...
        if(StringUtils.isEmpty(invokeSceneType)){
            log.warn("找不到对应的转换场景,providerClassName = "+invokeBean.getProviderClassName()
                    +",invokerClassName = "+invokeBean.getInvokerClassBean().getClassName()+",providerMethod = "+invokeBean.getProviderClassMethod().getMethodName()+",invokeMethod = "+invokeBean.getInvokerMethodBean().getMethodName());
            return;
        }
        //是应用层app_rpc作为调用方
