...
public class ClassConvertFactory {
    ...
    public List<ClassBean> getFacadeImplList(List<InterfaceBean> facadeInterfaceList) {
        ...
        for (InterfaceBean interfaceBean : facadeInterfaceList) {
            ...
            classBean.getImportClassList().addAll(interfaceBean.getChildClassImportPackageList());
            interfaceBean.getDynamicImportPackageList().forEach(str -> classBean.addImportClass(str));
    ...
    public List<InterfaceBean> getFacadeClassList(List<ClassBean> domainBOClassList) {
        ...
        facadeBOmap.forEach((k, v) -> {
            ...
            v.stream().forEach(classBean -> {
                ...
                classBean.getDynamicImportPackageList().stream().forEach(str-> facadeInterfaceBean.addDynamicImportClass(str));
    ...
    public List<ClassBean> getControllerClassList(List<ClassBean> domainBOClassList) {
        ...
        controllermap.forEach((k, v) -> {
            ...
            v.stream().forEach(classBean -> {
                ...
                classBean.getDynamicImportPackageList().stream().forEach(str -> controllerBean.addImportClass(str));
            });
            ...
            v.stream().forEach(classBean -> {
                ...
                if (fieldBeanOptional.isPresent()) {
                    ...
                    for (String voClassName : arr) {
                        ...
                        String volist2bolist = methodBeanFactory.getvolist2boListMethod(i.get());
                        ...
                        MethodBean boList2dtoList = new MethodBean();
                        String bolist2volist = methodBeanFactory.getbolist2voListMethod(i.get());
    ...
    /**
     * 处理派生类service->serviceimpl
     *
     * @param serviceInterfaceList
     * @return
     */
    public List<ClassBean> geServiceImplList(List<InterfaceBean> serviceInterfaceList) {
        List<ClassBean> serviceImplList = new ArrayList<>();

        for (InterfaceBean interfaceBean : serviceInterfaceList) {
            ClassBean classBean = new ClassBean();
            String className = interfaceBean.getClassName() + "Impl";
            classBean.setClassName(className);
            classBean.setClassDesc(classBean.getClassDesc());
            classBean.setPlantUMLPackage("infrast.serviceImpl");
